...
var loadTemplate = Git.safe(function loadTemplate(version, name, engine, callback) {
  Step(
    function loadHaml() {
      Git.readFile(version, "skin/" + name + "."+engine, this);
    },
    function compileTemplate(err, template) {
        if(err){ callback(err); return; }
        switch(engine){
        case "haml": 
            return Haml(template, (/\.xml$/).test(name)); 
        case "jade": 
            return function(data) { return Jade.render(template, {locals:data}); }; 
        default:
            callback("Couldn't load template for unknown engine "+engine);
            return;
        }
    },
    callback
...
function compileTemplate(name, engine, callback) {
  Step(
    function getHead() {
      Git.getHead(this);
    },
    function loadTemplates(err, version) {
      if (err) { callback(err); return; }
      loadTemplate(version, name, engine, this);
    },
...
function render(name, engine, data, callback, partial) {
  Step(
    function getHead() {
      Git.getHead(this);
    },
    function loadTemplates(err, version) {
      if (err) { callback(err); return; }
      loadTemplate(version, name, engine, this.parallel());
      if (!partial) {
        loadTemplate(version, "layout", engine, this.parallel());
      }
    },
