...
var loadTemplate = Git.safe(function loadTemplate(version, name, callback) {
  Step(
    function loadHaml() {
      Git.readFile(version, "skin/" + name + ".haml", this.parallel());
      Git.readFile(version, "skin/" + name + ".jade", this.parallel());
    },
    function compileTemplate(err, haml, jade) {
      if(haml){
        return Haml(haml, (/\.xml$/).test(name));
      } else if(jade){
          console.log('got jade: '+jade);
          return function(data) {
              return Jade.compile(jade)(data);
          };
      } else {
        callback(err);
        return;
