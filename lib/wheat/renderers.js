...
function insertSnippets(markdown, snippets, engine, callback) {
  Step(
    function () {
      Tools.compileTemplate('snippet', engine, this);
    },
...
var Renderers = module.exports = {
  index: Git.safe(function index() {
    // shift around args for optional page param
    var version, page, opts, callback;
    ...
    Step(
      ...
      function applyTemplate(err, articles, description, categories) {
        if (err) { callback(err); return; }
        Tools.render("index", opts.templateEngine, {
          articles: articles,
  ...
  feed: Git.safe(function feed(version, opts, callback) {
    var articles;
    Step(
      function loadData() {
      ...
      function (err, data) {
        ...
        articles.forEach(function (article) {
          insertSnippets(article.markdown, article.snippets, opts.templateEngine, group());
        });
      },
      function applyTemplate(err, markdowns) {
        ...
        Tools.render("feed.xml", opts.templateEngine, {
          articles: articles
        }, this, true);
      },
  ...
  article: Git.safe(function renderArticle(version, name, opts, callback) {
    ...
    Step(
      ...
      function (err, head, props) {
        if (err) { callback(err); return; }
        article = props;
        insertSnippets(article.markdown, article.snippets, opts.templateEngine, this.parallel());
        Git.readFile(head, "description.markdown", this.parallel());
      },
      function applyTemplate(err, markdown, description) {
        if (err) { callback(err); return; }
        article.markdown = markdown;
        Tools.render("article", opts.templateEngine, {
          title: article.title,
          article: article,
  ...
  categoryIndex: Git.safe(function index(version, category, opts, callback) {
    Step(
      function getHead() {
        Git.getHead(this);
      },
      ...
      function applyTemplate(err, articles, description, categories) {
        ...
        Tools.render("index", opt.templateEngine, {
  ...
  staticFile: Git.safe(function staticFile(version, path, opts, callback) {
    Step(
  ...
  dotFile: Git.safe(function dotFile(version, path, opts, callback) {
    Step(
      function loadPublicFiles() {
        Git.readFile(version, "skin/public/" + path, this);
      },
