...
var Renderers = module.exports = {
  index: Git.safe(function index(version, author, callback) {
    Step(
      function getHead() {
        Git.getHead(this);
      },
      function loadData(err, head) {
        ...
        Data.author(version, author, this.parallel());
      },
      function applyTemplate(err, articles, description, categories, author) {
        if (err) { callback(err); return; }
        Tools.render("index", {
          articles: articles,
