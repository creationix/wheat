  ...
  Step(
    ...
    function readFileContents(err, results) {
      ...
      files.forEach(function (filename) {
        Git.readFile(path.join("authors", filename), parallel());
      });
    },
    function parseFileContents(err) {
      if (err) return callback(err);
      var authors = {};
      Array.prototype.slice.call(arguments, 1).forEach(function (markdown, index) {
        var name = files[index].replace(/\.markdown$/, '');
        authors[name] = markdownPreParse(markdown);
      });
