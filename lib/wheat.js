var sys = require('sys');
require.paths.unshift(__dirname + "/wheat/deps");
require('proto');
...
function renderDot(path, callback) {
  Git.readFile("articles/" + path, function (err, data) {
    ...
    var dot = ChildProcess.spawn('dot', ["-Tpng"]);
    var stdout = "", stderr = "";
    dot.stdin.write(data);
    dot.stdout.setEncoding('binary');
    dot.stdout.addListener('data', function (text) {
      stdout += text;
    });
    dot.stderr.addListener('data', function (text) {
      stderr += text;
    });
    sys.p({
      stdout: stdout,
    ...
    dot.addListener('exit', function (code) {
      if (code > 0) {
